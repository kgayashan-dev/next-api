...
const EmployeesAndCars2 = () => {
  ...
  const handleEmployeeSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingEmployee) {
        ...
        response = await fetch(`http://localhost:5246/api/employees/${editingEmployee.id}`, {
          method: 'PUT',
      ...
      if (response.ok) {
        ...
        setErrorEmployee(`Failed to ${editingEmployee ? 'update' : 'add'} employee`);
      }
    } catch (error: any) {
      setErrorEmployee(error.message);
    }
  ...
  const handleCarSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCar) {
        // Update existing car
        response = await fetch(`http://localhost:5246/api/cars/${editingCar.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteEmployee = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:5246/api/employees/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-8 text-blue-800">Employees and Cars Management</h1>
